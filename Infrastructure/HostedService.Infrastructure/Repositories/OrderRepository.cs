...
public class OrderRepository(IOptions<CartSettings> cartOptions, IDistributedCache cache) : IOrderRepository
{
    ...
    public async Task SaveCachedOrdersToDbAsync()
    {
        ...
        var saveTasks = cartIds.Select(async cartId =>
        {
            ...
            try
            {
                await $"{_baseUrl}/orders/cache-to-db?cartId={cartId}".PostQuery();
            }
            finally
            {
                Semaphore.Release();
